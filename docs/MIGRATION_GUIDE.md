...
という進め方（11節のロードマップ4番目の具体化）がしやすくなる。また
...
Flutter化した画面が、移行前にネイティブが保存したローカルデータを読む必要が
ある場面は多い。このとき、**対応するFlutterプラグインをそのまま使っても既存
データは見えない**ことがある。プラグインが選ぶ保存先やキーの命名が、ネイティブ
の既定と一致しないためで、ストレージの種類ごとに事情が異なる。

| 種類 | 既存データをそのまま読めるか | 理由 |
|---|---|---|
| SQLite | 読める | DBファイルを絶対パスで開くだけ。Androidのデータベース既定ディレクトリはネイティブと共通 |
| キーバリューストア | 既定では読めない | 全キーに `flutter.` プレフィックスが付く。Androidは保存先ファイル名もFlutter専用のものになる |
| ファイル | Androidは要注意 | 「ドキュメント」に相当するディレクトリの解決先がネイティブと異なる |
| セキュアストレージ | 設定を合わせる必要がある | プラグインが独自のservice名・命名規則でアイテムを作る |

### キーバリューストア: プレフィックスとファイル名

既存のキーを読むには、プレフィックスを空にした上で、Androidでは保存先ファイル名
も明示する必要がある。プレフィックスの設定は**最初のインスタンスを取得するより
前**に行わなければ効かない。

さらに、プラグインが扱える型は真偽値・整数・浮動小数点数・文字列・文字列リスト
に限られる。既存のキーバリューストアに日付や辞書などが入っている場合はそのまま
では読めないため、対象キーを絞り込む許可リストの指定が必要になる。**既存アプリ
のストアには雑多な型が入っていることが多く、実務ではここが最初の障害になる。**

### ファイルパスの対応関係

ファイルの配置先を返すAPIは、OSごとに異なるネイティブのディレクトリへ解決される。

| Flutter側のAPI | Android | iOS |
|---|---|---|
| ドキュメントディレクトリ | Flutter専用のディレクトリ（ネイティブの内部ファイル領域とは**別**） | ネイティブのドキュメント領域と**同じ** |
| サポートディレクトリ | ネイティブの内部ファイル領域と**同じ** | ネイティブのサポート領域と同じ |

つまり、ネイティブが内部ファイル領域に置いたファイルをFlutterから読む場合、
Androidでは「ドキュメント」ではなく**「サポート」のAPI**を使う必要がある。
iOSはドキュメントで一致するため、**同じ意図のコードがOSによって別のAPIになる**
数少ない箇所であり、片方のOSだけで検証すると見落としやすい。

### SQLite

SQLiteは絶対パスでファイルを開くだけなので、既存のDBをそのまま利用できる。
引き継ぐ際に合わせる必要があるのは次の2点。

- **ファイルパス** — Androidのデータベース既定ディレクトリはネイティブの
  標準ヘルパークラスと共通なので、ファイル名を合わせるだけでよい。iOSは
  ネイティブ側が任意の場所に置いていることが多いため、実際の配置を確認する。
- **スキーマバージョン** — DBを開くときに指定するバージョン番号を既存の値に
  合わせる。ここを既定値のままにすると、既存DBに対して意図しないマイグレー
  ションが走る危険がある。
...
できる状態にすると、どちらが最新か分からない不整合が全面移行の完了まで残り
続ける。

- **移行期間中も双方が参照するデータ** — ネイティブを所有者のままにして、
  7節のパターンでFlutterへ渡す。プレフィックスを外して直接読むことはしない。
- **Flutterへ完全に移すデータ** — 起動時に一度だけ旧データを読んで新しい形式へ
  書き換え、移行済みフラグを立てて二度と読まない、という一方通行の移行にする。
  この場合は直接読みが適切。

### 確認方法

推測で進めず、実機・エミュレータ上のアプリデータディレクトリを直接確認する。
Androidはアプリのデータディレクトリ配下（データベース・設定ファイル・内部
ファイル領域が並んでいる）、iOSはアプリコンテナ配下（ドキュメント領域と
設定ファイルの保存先）を見れば、どのプラグインがどこに何を書いたかが分かる。
Flutter側で1件書き込んでからディレクトリを比較すると、ネイティブの保存先と
一致しているかを確実に判定できる。
