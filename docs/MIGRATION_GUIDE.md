...
という進め方（9節のロードマップ4番目の具体化）がしやすくなる。また
...
## 9. 全面Flutter化に向けたロードマップ（考え方）

1. **画面単位の置き換えを繰り返す。** 依存関係が少ない画面から着手し、
   徐々に依存の多い画面（認証必須の画面、共有状態を多く参照する画面など）へ
   広げる。
2. **モジュール構成を見直すタイミングを決めておく。** Flutter化した画面が
   増えてきたら、画面ごとに別モジュールのままにするか、1つのFlutterモジュール
   に複数画面（Flutter側のルーティング）を持たせるかを判断する。後者に
...
4. **共通基盤（通信・永続化・認証）をFlutter側に段階的に寄せる。** 個々の
   ...
   残り続ける。6節で説明したレイヤードアーキテクチャの構成であれば、
